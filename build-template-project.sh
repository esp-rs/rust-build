...
cargo generate --git https://github.com/esp-rs/esp-idf-template cargo --name test-std-$1 --vcs none --silent -d mcu=$1 -d advanced=false
cd test-std-$1
cargo build
# Build esp-tempalte
cargo generate -a esp-rs/esp-template --name test-nostd-$1 --vcs none --silent -d mcu=$1 -d advanced=false
cd test-nostd-$1
cargo build
